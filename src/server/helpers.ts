...
function removeDependenciesImpl(
    ...
    for (const dep of deps) {
        ...
        if (dependencyManager.getDependency(dep.uri) === undefined) { // No references left
            const depRes = dependenciesData.get(dep);
            if (depRes !== undefined) {
                removeDependenciesImpl(depRes.dependencies, dependencyManager, dependenciesData, visited);
...
function getReachableDependencies(
    ...
    for (const dep of depData.dependencies) {
        ...
        const depRes = dependenciesData.get(dep);
        if (depRes !== undefined) { reachableDeps.push(...getReachableDependencies(depRes, dependencyManager, dependenciesData, visited)); }
    }
...
export function removeUnreachableDependencies(
    ...
    for (const uri of unreachableDepsUris) {
        ...
        if (udep !== undefined) {
            removeDependencies(reachableDeps, dependencyManager, dependenciesData);
        }
    }
}