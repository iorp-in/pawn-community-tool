...
function readSpecicifers(content: string, position: number, initialToken: string): SpecifierResults {
    ...
    let tr: IdentifierResults | undefined = undefined;
    ...
    do {
        ...
        switch (tr.token) {
            ...
            case 'public':
                ...
                break;
            case 'hook':
                if (results.isHook || results.isStatic) {
                    results.wrongCombination = true;
                    break;
                }
                results.isHook = true;
    ...
    if (results.wrongCombination === true) {
        if (tr !== undefined) results.position = tr.position;
    }
    ...
    lines.forEach((lineContent, lineIndex) => {
        ...
        if (lineContent[0] === '#') {
            ...
            if (lineContent.substring(1, 8) === 'include' || lineContent.substring(1, 11) === 'tryinclude') {
                ...
                let charIndex = 0;
                let termCharacter: string | undefined;
                let filename = '';
            ...
            if (lineContent.indexOf('(') >= 0 && lineContent.indexOf(')') >= 0) { // Could be a callable
                ...
                let sr: SpecifierResults | undefined = undefined;
                switch (tr.token) {
                    case 'new':
                    case 'static':
                    case 'hook':
...
export function doSignatures(content: string, position: VSCLS.Position, callables: Types.CallableDescriptor[]): VSCLS.SignatureHelp | null {
    ...
    return {
        activeSignature: 0,
        activeParameter: Number(result.parameterIndex),
        signatures: [
            {
